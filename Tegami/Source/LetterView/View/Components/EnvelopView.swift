...
final class EnvelopView: UIView {
    ...
    private var letterName = "Cartas1" {
        didSet {
            DispatchQueue.main.async { [weak self] in
                if let letter = self?.letterImage {
                    UIView.transition(
                        ...
                        animations: {
                            letter.image = UIImage(named: self?.letterName ?? "Cartas1")
                        }
                    )
    ...
    private lazy var letterPinImage: UIImageView = {
        self.makeImageView(named: "selo", contentMode: .scaleAspectFit)
    }()
